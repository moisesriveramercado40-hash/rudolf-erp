...
// Helper para convertir Timestamp a Date (incluyendo dentro de arrays)
  ...
  for (const [key, value] of Object.entries(data)) {
    if (value instanceof Timestamp) {
      ...
      result[key] = value.map(item => {
        ...
        if (typeof item === 'object' && item !== null) return convertTimestamps(item);
        return item;
      });
...
// Helper para convertir Date a Timestamp (incluyendo dentro de arrays)
    ...
    if (value instanceof Date) {
      ...
      result[key] = value.map(item => {
        ...
        if (typeof item === 'object' && item !== null && !(item instanceof Date)) {
          return datesToTimestamps(item as Record<string, unknown>);
        }
        return item;
      });
    } else if (typeof value === 'object' && value !== null) {
...
export class FirebaseService<T extends { id: string }> {
  ...
  async getAll(): Promise<T[]> {
    try {
      ...
      // Fallback sin orderBy si el campo createdAt no existe
